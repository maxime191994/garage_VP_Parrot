...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <div class="container mt-4">
        <?php
        require_once __DIR__ . "/../controller/ServicesController.php"; // Assurez-vous que le chemin vers le contrôleur des services est correct
        require_once __DIR__ . "/../model/Service.php"; // Assurez-vous que le chemin vers le modèle de service est correct

        // Vérification de l'ID du service à supprimer
        ...
        $serviceId = $_GET['id'];
        $service = $servicesController->getService($serviceId);

        // Si le service n'existe pas, rediriger vers la liste
        if (!$service) {
            header("Location: list_services.php");
            exit;
        }
        ...
        if (isset($_POST['delete'])) {
            if ($servicesController->deleteService($serviceId)) {
                header("Location: list_services.php");
                exit;
            }
        }
        ...
        <form action="" method="POST">
            ...
            <a href="list_services.php" class="btn btn-secondary mb-4">Annuler</a>
        </form>
        <form action="list_services.php" method="GET">
            <button type="submit" class="btn btn-success mb-4">Retourner à la liste</button>
        </form>
    </div>
    <footer class="mt-4">
        <div class="container">
